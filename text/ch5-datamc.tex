...
Additionally, the necessity of reconstructing the neutrino energy renders $\thetaadt$ sensitive to neutrino flux uncertainties, which are among the largest systematic uncertainties in neutrino measurements~\cite{T2K:2019yqu,T2K:2021naz,MicroBooNECollaboration:2024gvg,NOvA:2023uxq,MINERvA:2022djk}.
     ...
     MC samples were generated using \genie \cite{Andreopoulos:2009rq, GENIE:2021npt} for the T2K beam and target. 
     ...
     \end{subfigure}
     \caption{$\thetacom$ distribution comparisons using the T2K flux on different nuclei and with multiple \genie configuations.}
     \label{fig:fsi-comp}
     \end{figure}
     ...
     \\
        ...
        \centering
        \includegraphics[width=\textwidth]{figures/COM/anorm-CH-mode11_da_tan.eps}  
        \caption{$\thetacom$ - $\deltapp$ decay.}
        \label{subfig:ch-comp-com-dpp}
    \end{subfigure}
    \begin{subfigure}[b]{\scfigwid\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/COM/anorm-CH-mode11_adt.eps} 
        \caption{$\thetaadt$ - $\deltapp$ decay.}
        \label{subfig:ch-comp-adt-dpp}
    \end{subfigure}
     \caption{Area normalized comparisons for FSI on/off for carbon and hydrogen with \texttt{G18\_10a\_02\_11b} between $\thetacom$ and $\thetaadt$ using the T2K flux and target. The bottom row has a more stringent selection to restrict the resonance to be $\deltapp$.}
     \label{fig:ch-comp}
     \end{figure}
     ...
     when a stricter selection is made to select only events involving $\deltapp$ as shown in Fig.~\ref{subfig:ch-comp-com-dpp}, the small difference in $\thetacom$ reduces further while the two curves deviate slightly more for $\thetaadt$ in fig.~\ref{subfig:ch-comp-adt-dpp}. 
     This suggests that in the absence of FSI, $\thetacom$ could be a better construction of $\thetapidel$ than $\thetaadt$. 
     ...
     \caption{Area normalized comparisons for different removal energies, $E_{\textrm{rm}}$, with the T2K flux.}
     \label{fig:ermc-comp}
     \end{figure}
     ...
     \end{subfigure}
     \caption{Comparisons for different $\MA$ values with the T2K flux. }
     \label{fig:ma-comp}
     \end{figure}
