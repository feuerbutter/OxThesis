   ...
   \end{figure}
    ...
    \section{Pion Trackless Reconstruction}
       ...
       \label{sec:tl-wp}
         ...
         A relation between the pion momentum, via its kinetic energy, and its range is obtained from an empirical fit using a PGUN sample, as shown in shown in \figref{fig:fit}.
          ...
          A fourth order polynomial is fitted to the central region of the plots, where the fitted line matches the data point well.
           ...
           \begin{figure}[h]
              ...
              \caption{A degree 4 polynomial fit of $\log_{10}{T_\pi}$ against $\log_{10}{L_\pi}$, where $T_\pi$ and $L_\pi$ are the kinetic energy and the range of the pion respectively. The $p_i$'s are the corresponding coefficients of the term of order $i$. The fit is satisfactory considering the close to 1 (\mccorrect{NOT TRUE}) $\chi^2/\textrm{n.d.f.}$ value. }
        ...
        \label{sec:tl-imp}
            ...
            However, they are insufficient as shown by the considerable portion of CC-other backgrounds in Figs.~\ref{subfig:tlpi-ppi-bf-trknumcut-tpc}, \ref{subfig:tlpi-tpi-bf-trknumcut-tpc}, \ref{subfig:tlpi-ppi-bf-trknumcut-sfg} and \ref{subfig:tlpi-tpi-bf-trknumcut-sfg}.
            ...
            The overall impact of the track number cuts is shown by the topological decomposition of the pion kinematic distributions in Fig.~\ref{fig:tl-trknum-cut-res}.
          ...
          As shown in Fig.~\ref{subfig:tlpi-ppi-bf-trknumcut-tpc} and Fig.~\ref{subfig:tlpi-ppi-af-trknumcut-tpc}, the purity of the $\numuccopi$ selection has improved from $74.1\%$ to $87.5\%$ after making the track number cuts for the TPC-$\mu$ sub-sample.
          Similarly, Fig.~\ref{subfig:tlpi-ppi-bf-trknumcut-sfg} and Fig.~\ref{subfig:tlpi-ppi-af-trknumcut-sfg} shows an improvement of selection purity from $65.9\%$ to $80.7\%$.
          Although the cut leads to about $18\%$ and $14\%$ decrease in the number of signal events, these results demonstrate that the track number cuts are effective in removing backgrounds.
          As the HAT-$\mu$ sub-sample still suffers from a global matching problem, the result is not included here.
          ...
          One quick consistency check is to plot the distribution of the time difference between the ME and the primary vertex, which should be the pion travel time plus the muon decay time.
          As the pion travel time i s on the order of nanosecond, the resultant distribution should still closely reproduce the muon exponential decay.
          The distributions of the ME time delay for the TPC-$\mu$ and SFGD-$\mu$ sub-samples are plotted in Fig.~\ref{tlpi-me-td}.
          \begin{figure}
               \centering
               \begin{subfigure}{\dbfigwid\textwidth}
    ...
    \section{Development of stopping pion control sample}
          ...
          \begin{figure}
               ...
               \end{subfigure}
              \caption{Pull distributions at HAT.}
               \label{fig:pulls}
            \end{figure}
          ...
          A further zoom on the first pull is shown in Fig.~\ref{fig:pull0-zoom}, which suggests a cut to exclude events with the first pull larger than $0.04$.
          \begin{figure}
               \centering
               \includegraphics[width=\sgfigwid\textwidth]{figures/sel/sspi_TOP_hat_pid0_stack_al5_zoom.eps}
               \caption{Zoom on the first pull distribution at HAT.}
               \label{fig:pull0-zoom}
          \end{figure}
          However, after the cut on the first pull, the other pull distributions do not have a good separation between the muon background and the pion signal anymore as shown in Fig.~\ref{fig:pulls-pid0}.
          \begin{figure}
               \centering
               \begin{subfigure}{\dbfigwid\textwidth}
            ...
            \end{figure}
          Instead, I incorporated an idea from a colleague, Xingyu Zhao, from his FGD analysis, where he defined a variable called the heavy length, which is the portion of the particle inside a dense detector, e.g. the SFGD.
          This length can distinguish between leptons and hadrons as hadrons are much more likely to interact with dense material than leptons, thereby having shorter lengths in these detectors.
          For the stopping pion sample, the heavy length is dominated by the length in the SFGD as the ECAL and the TOF could only contribute a small portion.
          As shown in Fig.~\ref{fig:sppi-heavylen-pid0}, a cut on the heavy length to keep events below $150~\mm$ can remove the a considerable portion of the muon background remained after the cut on the first pull.

          The pull distributions shown are based on the sub-sample where the pion passes through the Top HAT.
          The Bottom HAT has a similar performance and the same cuts are applied.
          For conciseness, the pull distributions for the Bottom HAT are collected in Sec.~\ref{sec:app-bot-pulls} in Apppendix.
          The final selection results for the two sub-samples are shown in Fig.~\ref{fig:sppi-kin}.
          The top HAT sub-sample has achieved a purity of $96\%$ as shown in Fig.~\ref{fig:sppi-top-ppi} and Fig.~\ref{fig:sspi-top-tpi}, which has $91$ $\pip$ signal events and $3$ $\mu^-$ and $2$ $\mu^+$ background events.
          Similarly, the bottom HAT sub-sample has achieved a purity of $87\%$ as shown in Fig.~\ref{fig:sppi-bot-ppi} and Fig.~\ref{fig:sspi-bot-tpi}, which has $293$ $\pip$ signal events and $15$ $\mu^-$, $26$ $\mu^+$ and $1$ $\pim$ background events.
          \begin{figure}
               \centering
               \begin{subfigure}{\dbfigwid\textwidth}
               ...
               \end{subfigure}
               \caption{Stopping pion kinematic distributions decomposition according to true particle type.}
               \label{fig:sppi-kin}
          \end{figure}

          As the HAT reconstruction is under development, the values implemented here might be subjected to further changes.
          A good consistentcy check of using the ME method to select the stopping pion is to check the time difference between the ME and the end point of the stopping track. 
          This distribution should reproduce the muon exponential decay curve closely.
