...
\label{sec:sel-esc}
   ...
   \label{sec:sel-esc-imp}
	...
	To investigate the quality of this estimate of $\dedx$, I have simulated multiple particle gun (PGUN) samples with protons traveling in different directions in the SFGD.
	...
	Another distinct feature of $\dedx$ is the minimal ionizing particle (MIP) region, where particles at high momentum traverse the scintillator depositing a minimal amount of energy, $E_{\textrm{MIP}}$, per unit distance.
     This is due to the fact that the energy loss of a particle traveling through matter is proportional to the square of its charge and inversely proportional to its velocity, as described by the Bethe-Bloch formula~\cite{Bethe:1930zza}.
     ...
     Hence, for protons traveling at high momentum, the $\dedx$ remains at a relatively constant and small value for all such particles.
	...
	Fig.~\ref{subfig:esc-smooth-e} plots the smoothed energy deposits for the third to sixth nodes from the end for all PGUN samples traveling at non-orthogonal angles.
   ...
   \end{figure}
    ...
    \section{Pion Trackless Reconstruction}
       ...
       \label{sec:tl-ps}
          ...
          \item \textbf{Others} – This category includes all uncategorized events, such as cases where the pion produces delta ray electrons or undergoes bremsstrahlung. 
    ...
    \section{Development of stopping pion control sample}
         ...
         % The TPC PID is based on the so-called "pulls", which essentially quantify the deviation
          ...
          The systematic error paramters from Table~\ref{tab:sppi-res} are then passed to the HighLAND framework, the selection development and systematics propagation software used at T2K, to propagate the uncertainty through the $\numuccopi$-TL selection.
